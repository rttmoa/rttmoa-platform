...
export type SearchProps = {
	...
	render?: (scope: SearchRenderScope) => Node // 自定义搜索内容渲染（tsx语法）
...
export interface ColumnProp<T = any> extends Partial<Omit<ColumnProps<T>, 'children' | 'render'>> {
	tag?: boolean // 是否是标签展示
	...
	headerRender?: (scope: HeaderRenderScope<T>) => Node // 自定义表头内容渲染（tsx语法）
	render?: (scope: RenderScope<T>) => Node | string // 自定义单元格内容渲染（tsx语法）
	_children?: ColumnProp<T>[] // 多级表头
}
