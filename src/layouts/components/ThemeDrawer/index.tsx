...
const ThemeDrawer: React.FC = () => {
	...
	return (
		<Drawer className='theme-drawer' title='主题配置' width={290} closable={false} maskClosable={true} open={themeDrawerVisible} onClose={() => dispatch(setGlobalState({ key: 'themeDrawerVisible', value: false }))}>
			{/* TODO: ====布局样式==== */}
			...
			<div className='layout-box'>
				<Tooltip placement='top' title='纵向' arrow={true} mouseEnterDelay={0.2}>
					<div className={`layout-item mb22 layout-vertical ${layout === 'vertical' && 'layout-active'}`} onClick={() => dispatch(setGlobalState({ key: 'layout', value: 'vertical' }))}>
				...
				<Tooltip placement='top' title='经典' arrow={true} mouseEnterDelay={0.2}>
					<div className={`layout-item mb22 layout-classic ${layout === 'classic' && 'layout-active'}`} onClick={() => dispatch(setGlobalState({ key: 'layout', value: 'classic' }))}>
						<div className='layout-dark'></div>
						<div className='layout-container'>
							<div className='layout-light'></div>
							<div className='layout-content'></div>
				...
				<Tooltip placement='top' title='横向' arrow={true} mouseEnterDelay={0.2}>
					<div className={`layout-item layout-transverse ${layout === 'transverse' && 'layout-active'}`} onClick={() => dispatch(setGlobalState({ key: 'layout', value: 'transverse' }))}>
						<div className='layout-dark'></div>
						<div className='layout-content'></div>
				...
				<Tooltip placement='top' title='分栏' arrow={true} mouseEnterDelay={0.2}>
					<div className={`layout-item layout-columns ${layout === 'columns' && 'layout-active'}`} onClick={() => dispatch(setGlobalState({ key: 'layout', value: 'columns' }))}>
						<div className='layout-dark'></div>
						<div className='layout-light'></div>
						<div className='layout-content'></div>
			...
			<div className='theme-item  mb-[35px]'>
				...
				<InputNumber
					...
					max={350}
					style={{ width: 80 }}
					defaultValue={sidebarWidth || 210}
					formatter={(value: any) => `${value}px`}
					parser={(value: any) => (value ? value!.replace('px', '') : 210) as number}
					onChange={(value: any) => {
						const newValue = value || 210;
						dispatch(setGlobalState({ key: 'sidebarWidth', value: newValue }));
					}}
				/>
			</div>
