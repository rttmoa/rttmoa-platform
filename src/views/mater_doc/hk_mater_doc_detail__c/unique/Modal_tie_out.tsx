...
type ModalTieOutProps = { open: boolean; selectedRow?: any; stockData?: any; onOpenChange: (open: boolean) => void; onSuccess?: () => void };

const ModalTieOut = ({ open, selectedRow, stockData, onOpenChange, onSuccess }: ModalTieOutProps) => {
	...
	const handleFinish = async (values: any) => {
		...
		try {
			...
			const res: any = await hk_mater_doc_detail__c_API.materTieOut(payload);
			const responseData = res?.data ?? {};
			if (responseData.success !== true) {
				message.error(responseData.message || res?.msg || '原料中铁出库失败', 5);
				return false;
			}
			message.success(responseData.message || res?.msg || '原料中铁出库成功');
			onSuccess?.();
			onOpenChange(false);
			form.resetFields();
			return true;
		} catch (error: any) {
			message.error(error?.message || error?.msg || '原料中铁出库失败', 5);
			return false;
		} finally {
			setSubmitting(false);
		}
