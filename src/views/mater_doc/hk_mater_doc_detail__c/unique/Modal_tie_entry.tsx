...
const ModalTieEntry = ({ open, selectedRow, onOpenChange, onSuccess }: ModalTieEntryProps) => {
	...
	const handleFinish = async (values: any) => {
		...
		try {
			...
			const res: any = await hk_mater_doc_detail__c_API.meterTieEnter(payload);
			const responseData = res?.data ?? {};
			if (responseData.success === false) {
				message.error(responseData.message || res?.msg || '原料中铁入库失败', 5);
				return false;
			}
			message.success(responseData.message || res?.msg || '原料中铁入库成功');
			onSuccess?.();
			onOpenChange(false);
			form.resetFields();
			return true;
		} catch (error: any) {
			message.error(error?.message || error?.msg || '原料中铁入库失败', 5);
			return false;
		} finally {
			setSubmitting(false);
		}
