...
const site = 1015;
const stockName = '保温库';
// await screen_API.find_keepwarm_out({ location: `${site}` })
...
const useProTable = () => {
	...
	const GetStockData = async () => {
		const data: any = await screen_API.find_keepwarm_out({ location: `${site}` });
		if (data?.code == 200) {
			...
			<Alert
				...
				message={
					<div className='w-full text-center font-sans font-bold text-[24px]'>
						所有出库任务：{total}个 —— 当前站点出库任务：{list.length}个
					</div>
				}
				type='success'
			/>
		...
		{isArrive && (
			<div>
				{[
					...
					['物料名称一一', data.material_name__c],
					['批号一一一一', data.batch__c],
					['生产日期一一', data.production_date__c],
					['是否整托出库', data.Whole], // 是整托出库 | 是半托出库
					['当前数量一一', data.now_quantity__c],
					['最终数量一一', data.final_quantity__c],
				].map(([label, value], idx) => {
					const totalChars = 8;
						...
						<Descriptions
							...
							title={
								<div className='flex text-[28px] font-bold font-mono'>
									<div className='w-[270px]'>{`${label}${filler}>`}</div>
									{value}
								</div>
							}
						/>
						...
						<div className='w-full'>
							...
							<div className='max-w-[95vw] xl:max-w-[1600px] text-[28px] font-bold leading-[1.3] flex flex-wrap gap-8 '>
								...
								<div className='flex flex-col justify-center items-center   xl:w-[200px] 2xl:w-[220px]'>
									...
									<span className='w-full whitespace-nowrap overflow-hidden text-ellipsis'>{`${item.loc_start__c} -> ${site}`}</span>
								</div>
