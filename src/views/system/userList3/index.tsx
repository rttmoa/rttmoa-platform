...
import './index.less';
import { fetchChangeUserStatus, fetchUserDelete, fetchUserDepttList, fetchUserDetail, fetchUserList, roleAPI } from '@/api/modules/system';
			...
			render: (text: any, record: { id: any; status: any }, index: any) => {
				return (
					<span>
						...
						{btnRights.freeze ? (
							<span>
								<Popconfirm title={`确认${record.status ? '解冻' : '冻结'}账户?`} placement='left' onConfirm={() => handleChangeStatus(record.id, `${record.status}`)}>
