...
const useProTableDynamic = ({ api, Virtual = false }: { api: any; Virtual?: boolean }) => {
	...
	const loadingStartRef = useRef<number>(undefined);
	...
	useEffect(() => {
		if (loading) {
			if (!loadingStartRef.current) {
				...
				loadingStartRef.current = timestamp;
				console.log(`[mater-stock-detail] 表格开始加载：${new Date(timestamp).toLocaleString()}`);
			}
			return;
		}

		let frameId: number;
		const finishWhenSpinnerStops = () => {
			const tableRoot = document.querySelector('.mater-stock-hover-table');
			const spinnerVisible = tableRoot?.querySelector('.ant-spin-spinning');
			if (spinnerVisible) {
				frameId = window.requestAnimationFrame(finishWhenSpinnerStops);
				return;
			}
			...
			const durationSeconds = (endTimestamp - loadingStart) / 1000;
			console.log(`[mater-stock-detail] 表格加载完成：${new Date(endTimestamp).toLocaleString()}，总耗时：${durationSeconds.toFixed(3)} 秒`);
	...
	const footerProps = { selectedRows, modalResult, allowBatchDelete: tableOps?.allowBatchDelete !== false };

	const showFooter = selectedRows?.length > 0;
