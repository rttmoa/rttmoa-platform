...
const duration = 1000;
const TransitionComponent = ({ i, v, remove, children, classNames, ...rest }: any) => (
  <CSSTransition {...rest} timeout={duration} classNames={classNames}>
    <div className="child">
      <Card style={{ width: "300px" }}>{v}</Card>
      <Button style={{ width: "80px" }} onClick={remove}>
        X
      </Button>
    </div>
  </CSSTransition>
...
export default class Slide extends React.Component {
  state = {
    ...
    rightList: ["right111"],
    topList: ["top111"],
    bottomList: ["bottom111"]
  };
  add = (value: string, key: any) => {
    // let list = this.state[key],
    //   state = this.state;
    // state[key] = [...list, value];
    // this.setState(state);
  };
  remove = (i: any, key: string | number) => {
    // let list = this.state[key],
    //   state = this.state;
    // list.splice(i, 1);
    // state[key] = list;
                ...
                <TransitionGroup>
                  {this.state.bottomList.map((v, i) => (
                    <TransitionComponent
                      key={i}
                      v={v}
                      remove={this.remove.bind(this, i, "bottomList")}
