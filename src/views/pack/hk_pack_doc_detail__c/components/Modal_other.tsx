...
const CREATE_FIELDS = ['area__c', 'material_code__c', 'batch__c', 'quantity__c', 'unit__c', 'type_move__c'];
...
const ModalComponent = (Params: any) => {
	...
	useEffect(() => {
		const loadMaterialOptions = async () => {
			try {
				...
				const res: any = await hk_mater_doc_detail__c_API.globalMaterial();
				const list = getResponseList(res);
				const nextMap: any = list.reduce((acc: Record<string, MaterialItem>, item: MaterialItem) => {
	...
	const loadCostCenterOptions = async () => {
		...
		try {
			...
			const res: any = await hk_mater_doc_detail__c_API.globalCostCenter();
	...
	const loadWarehouseOptions = async () => {
		...
		try {
			...
			const res: any = await hk_mater_doc_detail__c_API.globalWarehouseInfo();
	...
	const onFinish: any = (values: any) => {
		...
		if (modalType == 'create') {
			CREATE_FIELDS.concat(['cost_center__c', 'recept_area__c']).forEach(field => {
		...
		if (formList.batch__c) {
			formList.batch__c = String(formList.batch__c).trim();
		}
		...
		// formList.document_type__c = DOCUMENT_TYPE;
