...
const ModalEntry = ({ open, selectedRow, onOpenChange, onSuccess }: ModalEntryProps) => {
	...
	const handleFinish = async (values: any) => {
		...
		try {
			...
			const res: any = await hk_pack_doc_detail__c_API.packEntry(payload);
			const responseData = res?.data ?? {};
			if (responseData.success === false) {
				message.error(responseData.message || res?.msg || '包材库入库失败', 5);
				return false;
			}
			...
			message.error(error?.message || error?.msg || '包材库入库失败', 5);
			return false;
		} finally {
			setSubmitting(false);
		}
