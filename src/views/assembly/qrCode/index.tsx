import { Button, Card, Col, Input, message, Popover, QRCode, QRCodeProps, Row, Segmented, Space, theme } from 'antd'
import React, { useState } from 'react'
import CustomQR from './qrcode'
import './index.less'
		...
		<Row gutter={[12, 10]} className="qrCode-content">
			...
			<Col xl={6} lg={8} md={12} sm={24} xs={24}>
				<Card hoverable title="生成 Icon">
					<QRCode errorLevel="L" value="https://ant.design/" icon={src} />
				</Card>
			</Col>
			<Col xl={6} lg={8} md={12} sm={24} xs={24}>
			...
			<Col xl={6} lg={8} md={12} sm={24} xs={24}>
				<Card hoverable title="带气泡卡片">
					<Popover overlayInnerStyle={{ padding: 0 }} content={<QRCode value={src} bordered={false} />}>
						<img width={60} height={60} src={src} alt="icon" style={{ cursor: 'pointer' }} />
					</Popover>
				</Card>
			</Col>
