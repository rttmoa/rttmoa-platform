...
const TableDataImport = () => {
	...
	const [tableFile, setTableFile] = useState<Blob>(); // 正在上传的文件
	const [sheetNames, setSheetNames] = useState<sheetNamesSelectType[]>([]); // 工作表列表
	const [fileName, setFileName] = useState<string>(''); // 文件名称： 黑马 — PIP项目库账号信息.xlsx
	...
	const handleUpload = (file: Blob, sheetName?: string) => {
		...
		reader.onload = (e: any) => {
			...
			// 保存工作区
			setSheetNames(newSheetNames); // [{label: 'Sheet1', value: 'Sheet1'}]
			// 设置默认工作区
			...
			if (jsonData.length > 0) {
				...
				console.log('tableData', tableDataRes);
				// 将第一行转为表头
