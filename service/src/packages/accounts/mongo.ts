...
export class Mongo implements DatabaseInterface {
	...
	// 查找有效 session通过用户iD
	public async findValidSessionsByUserId(userId: string, is_phone: boolean): Promise<Array<Session>> {
		let query: any = {
			userId,
			valid: true,
		};
		...
		const sessions: any = await this.sessionCollection.find(query).project({ _id: 1 }).toArray();
		if (sessions) {
			sessions.forEach(function (session: { id: any; _id: { toString: () => any } }) {
	...
	public async createUser({ password, username, email, email_verified, mobile, mobile_verified, ...cleanUser }: CreateUser): Promise<string> {
	...
	public async setMobile(userId: string, newMobile: string): Promise<void> {
		...
		let existed = await this.collection.find({ _id: { $ne: id }, mobile: encryptedMobile }).count();
		if (existed > 0) {
			throw new Error("该手机号已被其他用户注册");
		}
		let user: any = await this.collection.findOne({ _id: id }, { projection: { mobile: 1 } });
	...
	public async createSession(userId: string, token: string, connection: ConnectionInformations = {}, extraData?: object): Promise<string> {
	...
	private async applyCode(name: any, owner: string, nextCode: string, MAX_FAILURE_COUNT: any, EFFECTIVE_TIME: any) {
	...
	public async addVerificationCode(user: any, code: string, options: any): Promise<void> {
		...
		else if (user.mobile) foundedUser = await this.findUserByMobile(user.mobile);
	...
	private async verifyCodeByName(name: any, code: string) {
		const now: any = new Date();
		let query = {
			name: name,
			code: code,
	...
	private async verifyCodeByOwner(owner: string, code: string) {
		const now: any = new Date();
		let query = {
			owner: owner,
			code: code,
	...
	public async updateMeteorSession(userId: string, token: string, infos: ConnectionInformations): Promise<boolean | null> {
		let when = new Date();
		const { login_expiration_in_days, is_phone, is_tablet } = infos;
		if (login_expiration_in_days && isNumber(login_expiration_in_days) && login_expiration_in_days > 0) {
			when = moment()
	...
	public async updateUser(userId: any, options: UpdateFilter<Document> | Partial<Document>) {
