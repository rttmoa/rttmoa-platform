...
import {
	...
	SteedosFormulaParamTypeConfig,
	SteedosFormulaVarPathTypeConfig,
	FormulaUserKey,
	SteedosFormulaOptions,
	SteedosQuotedByFieldFormulasTypeConfig,
} from "./type";
...
const runSFFormula = function (formula: string, params: Array<SteedosFormulaParamTypeConfig>, options?: SteedosFormulaOptions, messageTag?: any) {
	...
	if (result.type === "error") {
		...
		throw new Error(
			`runFormula:Catch an error "${result.message}" while eval formula "${formula}" with params: "${JSON.stringify(formulaParams)}" for "${JSON.stringify(messageTag)}"`
		);
		...
		switch (returnType) {
			case "boolean":
				if (formulaValueType !== FormulonDataType.Checkbox) {
					throw new Error(
						`runFormula:The field formula "${formula}" with params "${JSON.stringify(
							formulaParams
						)}" should return a boolean type result but got a ${formulaValueType} type value '${formulaValue}'.`
					);
				}
				break;
			case "number":
				if (formulaValueType !== FormulonDataType.Number) {
					throw new Error(
						`runFormula:The field formula "${formula}" with params "${JSON.stringify(
							formulaParams
						)}" should return a number type result but got a ${formulaValueType} type value '${formulaValue}'.`
					);
				}
				break;
			case "currency":
			...
			case "text":
				if (formulaValueType !== FormulonDataType.Text) {
					throw new Error(
						`runFormula:The field formula "${formula}" with params "${JSON.stringify(
							formulaParams
						)}" should return a string type result but got a ${formulaValueType} type value '${formulaValue}'.`
					);
				}
				break;
			case "date":
				if (formulaValueType !== FormulonDataType.Date) {
					...
					throw new Error(
						`runFormula:The field formula "${formula}" with params "${JSON.stringify(
							formulaParams
						)}" should return a date type result but got a ${formulaValueType} type value '${formulaValue}'.`
					);
				}
				break;
			case "datetime":
 ...
 */
export const updateQuotedByObjectFieldFormulaValue = async (
	objectName: string,
	recordId: string,
	fieldFormulaConfig: SteedosFieldFormulaTypeConfig,
	userSession: any,
	escapeConfigs?: Array<SteedosFieldFormulaTypeConfig> | Array<string>
) => {
...
export const updateDocsFieldFormulaValue = async (
	docs: any,
	fieldFormulaConfig: SteedosFieldFormulaTypeConfig,
	userSession: any,
	escapeConfigs?: Array<SteedosFieldFormulaTypeConfig> | Array<string>
...
export const updateQuotedByDocsForFormulaType = async (
	docs: any,
	fieldFormulaConfig: SteedosFieldFormulaTypeConfig,
	userSession: any,
	escapeConfigs?: Array<SteedosFieldFormulaTypeConfig> | Array<string>
