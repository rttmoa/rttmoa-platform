...
class App extends Basic {
	...
	private readonly tableName = '保温库世仓接口记录';
	private readonly Collection = 'kd_keepwarm_sc_record__c';

	private FieldSchema: FieldSchema = {
		// name: { label: '名称', type: 'string', sync: 'loc_name__c' },

		time__c: { label: '请求时间', type: 'string', width: 150, query: true, editable: true },
    
		req_name__c: { label: '请求名称', type: 'string',width:150, query: true, editable: true },
		req_address__c: { label: '请求地址', type: 'string',width:230,  query: true, editable: true },
    
		instruct_no__c: { label: '任务指令号', type: 'string', query: true, editable: true },

		status_code__c: { label: '响应状态码', type: 'string', query: true, editable: true },
		response_code__c: { label: '响应码', type: 'string', query: true, editable: true },
    ...
    is_success__c: {
			...
			editable: true,
			options: [
				{ label: '成功', value: '成功', color: '#99ff99' },
				{ label: '失败', value: '失败', color: '#ff8989' },
			],
		},

		response_message__c: { label: '响应消息', type: 'string',width:360, query: true, editable: true },
