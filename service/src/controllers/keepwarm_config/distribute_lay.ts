...
class App extends Basic {
	...
	private readonly tableName = '保温库货位层分配管理（优先）';
	private readonly Collection = 'kd_keepwarm_lay_distribution__c';

	private FieldSchema: FieldSchema = {
		// name: { label: '名称', type: 'string', sync: 'loc_name__c' },

		lay_manage__c: { label: '层管理', type: 'string', width:200, query: true, editable: true },
		open__c: {
			label: '开启/关闭',
			type: 'select',
			query: true,
			editable: true,
			options: [
				{ label: '开启', value: '开启', color: '#99ff99' },
				{ label: '关闭', value: '关闭', color: '#ff8989' },
			],
		},
		lay__c: { label: '层（数值）', type: 'number', query: true, editable: true, int: true },
