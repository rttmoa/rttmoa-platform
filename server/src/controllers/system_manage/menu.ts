...
/**
 * 将前端选择的 views 目录文件路径转换为菜单使用的相对路径。
 * 例如：.../src/views/_feature/auth/button/index -> /_feature/auth/button/index
 */
function normalizeMenuPath(value: unknown): string {
	const normalizedPath = String(value || "")
		.trim()
		.replace(/\\/g, "/");
	if (!normalizedPath) return "";

	const viewsDirectory = normalizedPath.match(/(?:^|\/)views?(?:\/|$)/i);
	if (!viewsDirectory || viewsDirectory.index === undefined) return normalizedPath;

	const pathAfterViews = normalizedPath.slice(viewsDirectory.index + viewsDirectory[0].length);
	return pathAfterViews ? `/${pathAfterViews.replace(/^\/+/, "")}` : "/";
}
...
class Menu extends Basic {
	...
	addMenu = async (ctx: Context) => {
		try {
			...
			const path = normalizeMenuPath(_.get(data, "path", ""));
			...
			let getSort = null;
			if (data?.parent_id != 0) {
				// 当不是顶级菜单时, 查询顶级菜单是否关闭，如果顶级菜单关闭，子菜单也需要关闭
				const exists = await ctx.mongo.find("__menu", { query: { key: data?.parent_id }, sort: { sort: -1 } });
				if (exists.length) {
					if (exists[0].enable == "关闭") enableStatus = "关闭";
				}
				const FindSort = await ctx.mongo.find("__menu", { query: { parent_id: data?.parent_id }, sort: { sort: -1 } });
				if (FindSort.length) {
					getSort = FindSort[0].sort + 10;
				} else {
					getSort = 10;
				}
			} else {
			...
			let fDoc: any = {
				...
				path, // 路由路径 /home/index | /auth/button
				element: normalizeMenuPath(data?.element), // 组件路径 /home/index | /auth/button/index
				redirect: normalizeMenuPath(data?.redirect), // 重定向路径，如 /auth/page
	...
	UpMenu = async (ctx: Context) => {
		try {
			...
			let fDoc: any = {
				...
				path: delStr(normalizeMenuPath(data?.path)), // 路由路径 /home/index | /auth/button
				element: delStr(normalizeMenuPath(data?.element)), // 组件路径 /home/index | /auth/button/index
				redirect: delStr(normalizeMenuPath(data?.redirect)), // 重定向路径，如 /auth/page
				key: delStr(data?.key),
	...
	DelMoreMenu = async (ctx: Context) => {
		try {
			...
			if (ids.length) {
				for (const id of ids) {
					const fm = await ctx.mongo.find("__menu", { query: { _id: id } });
					if (fm.length) {
						await ctx.mongo.deleteOne("__menu", fm[0]._id);
					}
				}
				...
				return ctx.sendError(400, "未选择数据，无法删除！");
			}
