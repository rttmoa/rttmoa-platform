...
export default class SteedosClient {
	...
	login = (user: string | object, password: string, token = "", deviceId = "") => {
		...
		return this.doFetch<UserProfile>(`${this.getAccountsRoute()}/password/login`, { method: "POST", body: JSON.stringify(body) });
	};
	...
	createUser = (user: UserProfile, token: string, inviteId: string, redirect: string) => {
		...
		const auth: any = this.doFetch<UserProfile>(`${this.getAccountsRoute()}/password/register${buildQueryString(queryParams)}`, { method: "POST", body: JSON.stringify(user) });
	...
	createSpace = (name: string) => {
		...
		const auth: any = this.doFetch<UserProfile>(`${this.getBaseRoute()}/api/v4/spaces/register/tenant${buildQueryString(queryParams)}`, { method: "POST", body: JSON.stringify({ name: name }) });
	...
	sendVerificationToken = (user: string) => {
		...
		return this.doFetch<UserProfile>(`${this.getAccountsRoute()}/password/sendVerificationCode`, { method: "POST", body: JSON.stringify(body) });
	};

	getSettings = () => {
		return this.doFetch<UserProfile>(`${this.getAccountsRoute()}/settings`, { method: "get" });
	};

	getMe = () => {
		return this.doFetch<UserProfile>(`${this.getAccountsRoute()}/user`, { method: "get" });
	};

	getMySpaces = () => {
		return this.doFetch<Space[]>(`${this.getAccountsRoute()}/user/spaces`, { method: "get" });
	};
	...
	changePassword = (oldPassword: string, newPassword: string) => {
		return this.doFetch<UserProfile>(`${this.getAccountsRoute()}/password/changePassword`, {
			method: "POST",
			body: JSON.stringify({
	...
	verifyEmail = (email: string, code: string) => {
		return this.doFetch<UserProfile>(`${this.getAccountsRoute()}/password/verify/email`, {
			method: "POST",
			body: JSON.stringify({
				email: email,
				code: code,
	...
	verifyMobile = (mobile: string, code: string) => {
		return this.doFetch<UserProfile>(`${this.getAccountsRoute()}/password/verify/mobile`, {
			method: "POST",
			body: JSON.stringify({
				mobile: mobile,
				code: code,
